...
RUN cd oar-skylight && perl -pi -e "s;API: 'localhost:46668;API: 'luke-api.univ-grenoble-alpes.fr;" src/environments/environment.prod.ts
RUN cd oar-skylight && npm install
RUN cd oar-skylight && npm update
RUN cd oar-skylight && npm run build

# To build oar-skylight, you can run:
#     docker build -t skylight .
#     docker run -v /var/www/oar-skylight:/server -it skylight
