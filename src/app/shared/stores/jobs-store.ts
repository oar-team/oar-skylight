...
export class JobsStore {
  ...
  updateJobs(): void {
    ...
    if (jobIds.length > 0) {
      this.oarApiService.getJobsById(jobIds).subscribe((json: any) => {
        if (json.items.length > 0) {
          json.items.forEach((jsonJob: any) => {
            let jobsInStore = this._jobs.getValue();

            if (
              jobsInStore.find(job => job.id == +jsonJob.id).state !=
              jsonJob.state
            ) {
  ...
  updateJob(id: string) {
    this.oarApiService.getJob(id).subscribe(jsonJob => {
  ...
  initNewJobs(json: any) {
    ...
    if (json.items.length > 0) {
      json.items.forEach((jsonJob: any) => {
        let job: Job = new Job().deserialize(jsonJob);
        if (
          !this.containsJob(job.id.toString(), this._jobs.getValue().toArray())
        ) {
          this.addJobWithJob(job, this._jobs.getValue().toArray());
        }
      });
  ...
  getJob(id: string): Job {
    if (this.containsJob(id, this._jobs.getValue().toArray())) {
      return this._jobs.getValue().find(job => job.id.toString() === id);
    } else {
      console.log("else");
   ...
   * @memberof JobsStore
   */
  getJobResources(id: number): Observable<Response> {
    return this.oarApiService.getJobResources(id.toString());
  }
  ...
  containsJob(id: string, jobs: Array<Job>): boolean {
    return jobs.some(job => job.id.toString() === id);
  }
